...
<body>
    ...
    <div style="padding-left: 20px;">
        <form class="layui-form layui-form-pane" id="frmBasic">
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline marginTop4px">
                    <input type="text" name="AssessmentDate" id="AssessmentDate" lay-verify="date" value='<%=DateTime.Now.ToString("yyyy-MM-dd") %>' autocomplete="off" class="layui-input" />
    ...
    <div style="padding-left: 20px; padding-right: 20px;">
        ...
        <form id="frmOperation">
                            ...
                            <ItemTemplate>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input type="text" value='<%# GetUserScore(Eval("ID")) %>' maxlength="5" lay-verify="required|number" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" placeholder="得分" onblur="this.v();" autocomplete="off" class="layui-input" <%if (lstOperationRecordInfo[0].IsOperationApproved) Response.Write("disabled='disabled'"); %>>
                                    </td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
                        ...
                        <asp:Repeater runat="server" ID="rptLean" OnItemDataBound="RptOperation_ItemDataBound">
                            <ItemTemplate>
                                <tr>
                                    ...
                                    <td>
                                        <input type="hidden" value='<%# Eval("ID") %>' />
                                        <input type="text" value='<%# GetUserScore(Eval("ID")) %>' name='<%# Eval("ID") %>' maxlength="5" lay-verify="required|number" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" placeholder="得分" onblur="this.v();" autocomplete="off" class="layui-input" <%if (lstOperationRecordInfo[0].IsOperationApproved) Response.Write("disabled='disabled'"); %>>
                                    </td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
            ...
            <div class="layui-form-item">
                <% if (lstOperationRecordInfo[0].IsOperationApproved) Response.Write(""); else Response.Write("<input id='btnSaveOperation' class='layui-btn' value='提交' type='button' /><span style='color:red;margin-left:10px;' >提交后，如果实操、精益、其它下面的子项不填写得分，将会默认以0分提交。此处提交只限操作一次，不可修改。</span>"); %>
            </div>
        </form>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <blockquote class="layui-elem-quote">
            ...
            <span style="color: #0B588A;">总分 = 理论知识得分 X 15% + 实际操作得分 X 50% + 精益分 X 20% + 其它分 X 15%</span>
