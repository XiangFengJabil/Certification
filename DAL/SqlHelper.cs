...
namespace Certification.DAL
{
    public class SqlHelper
    {
        ...
        {
            int iRows = 0;
            SqlConnection myConn = new SqlConnection(CONN_STRING_DEFAULT);
            myConn.Open();
            //事务
            SqlTransaction trans = myConn.BeginTransaction();
            SqlBulkCopy bulkCopy = new SqlBulkCopy(myConn, SqlBulkCopyOptions.Default, trans);
        ...
        /// <summary>
        /// 使用事务将datatable批量插入，数据库主键不用在dt参数中,成功所回dt的Rows,否则失败。
        /// </summary>
        /// <param name="dt">需要插入的datatable</param>
        /// <param name="databaseTableName">数据库表名</param>
        /// <returns></returns>
        public static int SqlbulkcopyInsert(DataTable dt, string databaseTableName)
        {
            int iRows = 0;
            using (SqlConnection conn = new SqlConnection(CONN_STRING_DEFAULT))
            {
                conn.Open();
        ...
        /// 事务处理
        /// </summary>
        /// <param name="lst">insert,update,delete
        /// insert into table 
        /// </param>
        /// <returns></returns>
        public static int ExecuteNonQueryTran(List<string> lst)
        {
            int iRow = 0;

            SqlConnection sqlConnection = new SqlConnection(CONN_STRING_DEFAULT);
            sqlConnection.Open();
            ...
            try
            {
                ...
                {
                    cmd.CommandText = lst[i];
                    iRow = iRow + cmd.ExecuteNonQuery();
                }
                //myTrans.Rollback();
                myTrans.Commit();
            }
